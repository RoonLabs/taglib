...
class DSFHeader
{
 public:
  ...
  inline unsigned long long bytesToUInt64(const char *v, unsigned long long offset = 0) {
    unsigned long long res = 0;
    for (int i = 0; i < LONG_INT_SIZE; i++) {
      res |= static_cast<unsigned long long>(static_cast<unsigned char>(v[offset + i])) 
	<< (i * 8);
    }
    return res;
  }
