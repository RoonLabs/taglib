...
public:
  ...
  unsigned long long ID3v2Location; // For DSD it's always > 0 if tag is present
  unsigned long long ID3v2OriginalSize;
  ...
  static inline TagLib::ByteVector& uint64ToVector(unsigned long long num, 
						   TagLib::ByteVector &v) 
  {
    char raw[8];
...
bool DSFFile::save(int id3v2Version, bool shrink)
{
  ...
  if(ID3v2Tag() && !ID3v2Tag()->isEmpty()) {
    ...
    unsigned long long fileSize = d->fileSize + id3v2_v.size() - d->ID3v2OriginalSize;
		   ...
		   TagLib::AudioProperties::ReadStyle propertiesStyle)
{
  ...
  if (d->properties->sampleRate() == 0) {
      delete d->properties;
      d->properties = 0;
      return; // didn't get sane properties
  }
